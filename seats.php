...
$pdo->prepare(
    "UPDATE showtime_seats
     ...
     WHERE showtime_id = ? AND status = 'locked' AND locked_until < (NOW() - INTERVAL 10 MINUTE)"
)->execute([$showtimeId]);

$stmt = $pdo->prepare(
    'SELECT ss.id AS showtime_seat_id, se.seat_row, se.seat_number, se.seat_type, se.is_wheelchair, ss.status, ss.locked_until
